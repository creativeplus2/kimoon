<div class="row mb-2">
    ...
    <div class="col-md-6 mb-2">
        ...
        <input type="text" name="no_telpon" id="no-telpon" class="form-control @error('no_telpon') is-invalid @enderror"
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('type_user') is-invalid @enderror" name="type_user"
            id="type-user" required>
            <option value="" selected disabled>-- {{ __('Select type user') }} --</option>
            <option value="Reseller" {{ isset($member) && $member->type_user == 'Reseller' ? 'selected' :
                (old('type_user') == 'Reseller' ? 'selected' : '') }}>
                Reseller</option>
            <option value="Subdis" {{ isset($member) && $member->type_user == 'Subdis' ? 'selected' : (old('type_user')
                == 'Subdis' ? 'selected' : '') }}>
                Subdis</option>
            <option value="Distributor" {{ isset($member) && $member->type_user == 'Distributor' ? 'selected' :
                (old('type_user') == 'Distributor' ? 'selected' : '') }}>
                Distributor</option>
        </select>
        @error('type_user')
        <span class="text-danger">
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('provinsi_id') is-invalid @enderror"
            ...
            <option value="{{ $province->id }}" {{ isset($member) && $member->provinsi_id == $province->id ? 'selected'
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('kabkot_id') is-invalid @enderror" name="kabkot_id"
            ...
            <option value="{{ $kabkot->id }}" {{ isset($member) && $member->kabkot_id == $kabkot->id ? 'selected' :
                (old('kabkot_id') == $kabkot->id ? 'selected' : '') }}>
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('kecamatan_id') is-invalid @enderror"
            ...
            <option value="{{ $kecamatan->id }}" {{ isset($member) && $member->kecamatan_id == $kecamatan->id ?
                'selected' : (old('kecamatan_id') == $kecamatan->id ? 'selected' : '') }}>
                {{ $kecamatan->kecamatan }}
            </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('kelurahan_id') is-invalid @enderror"
            name="kelurahan_id" id="kelurahan-id" required>
            <option value="" selected disabled>-- {{ __('Select kelurahan') }} --</option>

            @foreach ($kelurahans as $kelurahan)
            <option value="{{ $kelurahan->id }}" {{ isset($member) && $member->kelurahan_id == $kelurahan->id ?
    ...
    <div class="col-md-6 mb-2">
        ...
        <input type="text" name="zip_code" id="zip-code" class="form-control @error('zip_code') is-invalid @enderror"
            value="{{ isset($member) ? $member->zip_code : old('zip_code') }}" readonly
    ...
    <div class="col-md-6 mb-2">
        ...
        <input type="text" name="no_ktp" id="no-ktp" class="form-control @error('no_ktp') is-invalid @enderror"
            value="{{ isset($member) ? $member->no_ktp : old('no_ktp') }}" placeholder="{{ __('No Ktp') }}" required />
        @error('no_ktp')
        <span class="text-danger">
            {{ $message }}
        </span>
        ...
        <div class="row">
            <div class="col-md-4 text-center">
                ...
                <img src="https://via.placeholder.com/350?text=No+Image+Avaiable" alt="Photo Ktp"
                    class="rounded mb-2 mt-2" alt="Photo Ktp" width="200" height="150" style="object-fit: cover">
                @else
                <img src="{{ asset('storage/uploads/photo_ktps/' . $member->photo_ktp) }}" alt="Photo Ktp"
                    class="rounded mb-2 mt-2" style="width: 100%">
                @endif
            </div>
    ...
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('status_member') is-invalid @enderror"
            name="status_member" id="status-member" required>
            <option value="" selected disabled>-- {{ __('Select status member') }} --</option>
            <option value="Pending" {{ isset($member) && $member->status_member == 'Pending' ? 'selected' :
                (old('status_member') == 'Pending' ? 'selected' : '') }}>
                Pending</option>
            <option value="Approved" {{ isset($member) && $member->status_member == 'Approved' ? 'selected' :
                (old('status_member') == 'Approved' ? 'selected' : '') }}>
                Approved</option>
            <option value="Rejected" {{ isset($member) && $member->status_member == 'Rejected' ? 'selected' :
                (old('status_member') == 'Rejected' ? 'selected' : '') }}>
                Rejected</option>
        </select>
        @error('status_member')
        <span class="text-danger">
