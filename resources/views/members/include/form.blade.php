<div class="row mb-2">
    <div class="col-md-6 mb-2">
            ...
            <input type="text" name="kode_member" id="kode-member" class="form-control @error('kode_member') is-invalid @enderror" value="{{ isset($member) ? $member->kode_member : old('kode_member') }}" placeholder="{{ __('Kode Member') }}" required />
            @error('kode_member')
...
<div class="col-md-6 mb-2">
                               <label for="type-user">{{ __('Type User') }}</label>
                                    <select class="form-control @error('type_user') is-invalid @enderror" name="type_user" id="type-user"  required>
                                        ...
                                        <option value="Seller" {{ isset($member) && $member->type_user == 'Seller' ? 'selected' : (old('type_user') == 'Seller' ? 'selected' : '') }}>Seller</option>
		<option value="Subdis" {{ isset($member) && $member->type_user == 'Subdis' ? 'selected' : (old('type_user') == 'Subdis' ? 'selected' : '') }}>Subdis</option>
		<option value="Distributor" {{ isset($member) && $member->type_user == 'Distributor' ? 'selected' : (old('type_user') == 'Distributor' ? 'selected' : '') }}>Distributor</option>			
                                    </select>
                                    @error('type_user')
                                        <span class="text-danger">
                                            {{ $message }}
...
<div class="col-md-6 mb-2">
                        ...
                        @foreach ($kabkots as $kabkot)
                            <option value="{{ $kabkot->id }}" {{ isset($member) && $member->kabkot_id == $kabkot->id ? 'selected' : (old('kabkot_id') == $kabkot->id ? 'selected' : '') }}>
                                {{ $kabkot->ibukota }}
                            </option>
                        @endforeach
                                    </select>
...
<div class="col-md-6 mb-2">
                        ...
                        @foreach ($kecamatans as $kecamatan)
                            <option value="{{ $kecamatan->id }}" {{ isset($member) && $member->kecamatan_id == $kecamatan->id ? 'selected' : (old('kecamatan_id') == $kecamatan->id ? 'selected' : '') }}>
                                {{ $kecamatan->id }}
                            </option>
                        @endforeach
                                    </select>
...
<div class="col-md-6 mb-2">
                        ...
                        @foreach ($kelurahans as $kelurahan)
                            <option value="{{ $kelurahan->id }}" {{ isset($member) && $member->kelurahan_id == $kelurahan->id ? 'selected' : (old('kelurahan_id') == $kelurahan->id ? 'selected' : '') }}>
                                {{ $kelurahan->id }}
                            </option>
                        @endforeach
                                    </select>
    ...
    <div class="col-md-6 mb-2">
            ...
            <input type="text" name="zip_code" id="zip-code" class="form-control @error('zip_code') is-invalid @enderror" value="{{ isset($member) ? $member->zip_code : old('zip_code') }}" placeholder="{{ __('Zip Code') }}" required />
            @error('zip_code')
                <span class="text-danger">
                    {{ $message }}
                </span>
...
<div class="col-md-6 mb-2">
                               <label for="status-member">{{ __('Status Member') }}</label>
                                    <select class="form-control @error('status_member') is-invalid @enderror" name="status_member" id="status-member"  required>
                                        <option value="" selected disabled>-- {{ __('Select status member') }} --</option>
                                        <option value="Pending" {{ isset($member) && $member->status_member == 'Pending' ? 'selected' : (old('status_member') == 'Pending' ? 'selected' : '') }}>Pending</option>
		<option value="Approved" {{ isset($member) && $member->status_member == 'Approved' ? 'selected' : (old('status_member') == 'Approved' ? 'selected' : '') }}>Approved</option>
		<option value="Rejected" {{ isset($member) && $member->status_member == 'Rejected' ? 'selected' : (old('status_member') == 'Rejected' ? 'selected' : '') }}>Rejected</option>			
                                    </select>
                                    @error('status_member')
                                        <span class="text-danger">
                                            {{ $message }}
