<div class="row mb-2">
    <div class="col-md-6 mb-2">
        ...
        <select class="form-control js-example-basic-multiple @error('bank_id') is-invalid @enderror" name="bank_id" id="bank-id" required>
            <option value="" selected disabled>-- {{ __('Select bank') }} --</option>

            @foreach ($banks as $bank)
                <option value="{{ $bank->id }}"
                    {{ isset($accountBank) && $accountBank->bank_id == $bank->id ? 'selected' : (old('bank_id') == $bank->id ? 'selected' : '') }}>
                    {{ $bank->nama_bank }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-2">
        ...
        <input type="text" name="account_number" id="account-number"
            ...
            value="{{ isset($accountBank) ? $accountBank->account_number : old('account_number') }}"
    ...
    <div class="col-md-6 mb-2">
        ...
        <input type="text" name="account_name" id="account-name"
            ...
            value="{{ isset($accountBank) ? $accountBank->account_name : old('account_name') }}"
